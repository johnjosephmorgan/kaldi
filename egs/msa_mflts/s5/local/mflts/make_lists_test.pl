...
# make_lists_test.pl - write  Kaldi IO lists
# writes files under data/local/tmp/mflts/test/lists
...
my $tmpdir = "data/local/tmp/mflts/test";
...
open my $SEG, '+>', $segs or croak "problem with $segs $!";
 ...
 my $rec_id = $base;
  ...
  print $WAVSCP "$rec_id sox -r 44100 -b 16 -e signed \"$wav_file{$base}\" -r 16000 -b 16 -e signed -t .wav - remix $rmx |\n";
