...
# make_lists_test_nonnative.pl - write lists for acoustic model training
# writes files under data/local/tmp/ru/test_nonnative/lists
...
my $tmpdir = "data/local/tmp/ru/test_nonnative";
...
LINEA: while ( my $line = <$TR> ) {
  ...
  my ($gendre,$spk,$utt,$country) = split /\-/, $file, 4;
...
LINE: while ( my $line = <$W> ) {
  ...
  my ($gendre,$spk_num,$country,$utt) = split /\_/, $base, 4;
  my $spk = $gendre . '_' . $spk_num . '_' . $country;
