...
# make_lists_test_native.pl - write lists for acoustic model training
# writes files under data/local/tmp/ru/test_native/lists
...
my $tmpdir = "data/local/tmp/ru/test_native";
...
LINEA: while ( my $line = <$TR> ) {
  ...
  my ($gendre,$spk,$country,$utt) = split /\-/, $file, 4;
