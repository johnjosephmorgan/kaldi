...
if [ $stage -le 3 ]; then
  ...
  utils/combine_data.sh data/transtac_read/train \
    $read_dir/appen/2005/lists \
    $read_dir/appen/2006/lists \
    $read_dir/ma/2006/lists
fi

if [ $stage -le 4 ]; then
  echo "$0: Extract features from Transtac Read data."
  steps/make_mfcc.sh --cmd "$train_cmd" --nj 56 data/transtac_read/train
  steps/compute_cmvn_stats.sh data/transtac_read/train
  utils/fix_data_dir.sh data/transtac_read/train
fi

if [ $stage -le 5 ]; then
  echo "$0: Train monophones on transtac read Speech."
  steps/train_mono.sh  --cmd "$train_cmd" --nj 56 data/transtac_read/train \
		       data/lang exp/transtac_read/mono || exit 1;
fi

if [ $stage -le 6 ]; then
  echo "$0: aligning with transtac read monophones"
  steps/align_si.sh  --cmd "$train_cmd" --nj 56 data/transtac_read/train \
    data/lang exp/transtac_read/mono exp/transtac_read/mono_ali || exit 1;
fi

if [ $stage -le 7 ]; then
  echo "$0: Starting  transtac read triphone training in exp/transtac_read/tri1."
...
if [ $stage -le 8 ]; then
  ...
  steps/align_si.sh  --cmd "$train_cmd" --nj 56 data/transtac_read/train \
    data/lang exp/transtac_read/tri1 exp/transtac_read/tri1_ali || exit 1;
fi
...
if [ $stage -le 9 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" \
    ...
    5500 90000 \
    data/transtac_read/train data/lang exp/transtac_read/tri1_ali \
    exp/transtac_read/tri2b || exit 1;
fi
...
if [ $stage -le 10 ]; then
  ...
  steps/align_si.sh  --nj 56 \
    ...
    exp/transtac_read/tri2b exp/transtac_read/tri2b_ali || exit 1;
fi
...
if [ $stage -le 11 ]; then
  ...
  steps/train_sat.sh --cmd "$train_cmd" \
    5500 90000 \
    data/transtac_read/train data/lang exp/transtac_read/tri2b_ali \
    exp/transtac_read/tri3b || exit 1;
fi
