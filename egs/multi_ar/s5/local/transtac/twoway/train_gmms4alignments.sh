...
if [ $stage -le 7 ]; then
  ...
  utils/fix_data_dir.sh data/transtac_twoway/train || exit 1;
fi
...
if [ $stage -le 8 ]; then
  ...
  steps/train_mono.sh  --cmd "$train_cmd" --nj 56 data/transtac_twoway/train \
		       data/lang exp/transtac_twoway/mono || exit 1;
fi

if [ $stage -le 6 ]; then
  echo "$0: aligning with transtac read monophones"
  steps/align_si.sh  --cmd "$train_cmd" --nj 56 data/transtac_twoway/train data/lang \
...
if [ $stage -le 7 ]; then
  ...
  steps/train_deltas.sh --cmd "$train_cmd" --boost-silence 1.25 \
    5500 90000 \
    data/transtac_twoway/train data/lang exp/transtac_twoway/mono_ali exp/transtac_twoway/tri1 || exit 1;
fi
...
if [ $stage -le 9 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" \
    ...
    5500 90000 \
    data/transtac_twoway/train data/lang exp/transtac_twoway/tri1_ali exp/transtac_twoway/tri2b || exit 1;
fi
...
if [ $stage -le 10 ]; then
  ...
  steps/align_si.sh  --nj 56 \
    --cmd "$train_cmd" \
    --use-graphs true data/transtac_twoway/train data/lang exp/transtac_twoway/tri2b \
...
if [ $stage -le 11 ]; then
  ...
  steps/train_sat.sh --cmd "$train_cmd" \
    5500 90000 \
    data/transtac_twoway/train data/lang exp/transtac_twoway/tri2b_ali exp/transtac_twoway/tri3b || exit 1;
fi
