...
# run those things.
local/tamsa/nnet3/run_ivector_common.sh --stage $stage \
                                  --train-set $train_set \
                                  --gmm $gmm \
...
lat_dir=exp/tamsa/chain${nnet3_affix}/${gmm}_${train_set}_sp_lats
dir=exp/tamsa/chain${nnet3_affix}/tdnn${affix}_sp
train_data_dir=data/tamsa/${train_set}_sp_hires
...
if [ $stage -le 10 ]; then
  ...
  if [ -d $lang ]; then
    if [ $lang/L.fst -nt data/tamsa/lang/L.fst ]; then
      echo "$0: $lang already exists, not overwriting it; continuing"
    else
      echo "$0: $lang already exists and seems to be older than data/tamsa/lang..."
      echo " ... not sure what to do.  Exiting."
      exit 1;
    fi
  else
    cp -r data/tamsa/lang $lang
...
if [ $stage -le 11 ]; then
  ...
  steps/align_fmllr_lats.sh --nj 20 --cmd "$train_cmd" ${lores_train_data_dir} \
    data/tamsa/lang $gmm_dir $lat_dir
...
if [ $stage -le 16 ]; then
  ...
  for data in $test_sets; do
    (
    nspk=$(wc -l <data/tamsa/${data}_hires/spk2utt)
    steps/nnet3/decode.sh \
      ...
      --cmd "$decode_cmd" \
      --num-threads 4 \
      --online-ivector-dir exp/tamsa/nnet3${nnet3_affix}/ivectors_${data}_hires \
      $tree_dir/graph \
      data/tamsa/${data}_hires \
...
if $test_online_decoding && [ $stage -le 17 ]; then
  ...
  steps/online/nnet3/prepare_online_decoding.sh \
    ...
    $lang \
    exp/tamsa/nnet3${nnet3_affix}/extractor \
    ${dir} \
    ${dir}_online
  ...
  for data in $test_sets; do
    (
    nspk=$(wc -l <data/tamsa/${data}_hires/spk2utt)
    # note: we just give it "data/tamsa/${data}" as it only uses the wav.scp, the
    # feature type does not matter.
    steps/online/nnet3/decode.sh \
      --acwt 1.0 --post-decode-acwt 10.0 \
      --nj $nspk --cmd "$decode_cmd" \
      $tree_dir/graph data/tamsa/${data} ${dir}_online/decode_${data} || exit 1
    ) || touch $dir/.error &
  done
  wait
  [ -f $dir/.error ] && echo "$0: there was a problem while decoding" && exit 1
