...
perl -MList::Util=shuffle -e 'print shuffle(<STDIN>);' < $corpus | \
     head -n $nsegs > data/tamsa/lm/train_small.txt
...
ngram-count -order 3 -interpolate -unk -map-unk "<UNK>" \
    -limit-vocab -text data/tamsa/lm/train_small.txt -lm data/tamsa/lm/tgsmall.arpa || exit 1;

ngram-count -order 3 -interpolate -unk -map-unk "<UNK>" \
  -limit-vocab -text data/tamsa/lm/train_small.txt \
  -prune 0.0000001 -lm data/tamsa/lm/tgsmall.pruned.arpa || exit 1;

gzip -f data/tamsa/lm/tgsmall.arpa
gzip -f data/tamsa/lm/tgsmall.pruned.arpa
