...
for f in data/tamsa/${train_set}/feats.scp ${gmm_dir}/final.mdl; do
  if [ ! -f $f ]; then
...
if [ $stage -le 1 ]; then
	...
	data/tamsa/${train_set} \
	data/tamsa/${train_set}_sp
...
if [ $stage -le 3 ]; then
    ...
    mfccdir=data/tamsa/${train_set}_sp_hires/data
    for datadir in ${train_set}_sp ${test_sets}; do
	utils/copy_data_dir.sh \
	    data/tamsa/$datadir \
	    data/tamsa/${datadir}_hires
    done

    # do volume-perturbation on the training data prior to extracting hires
    # features; this helps make trained nnets more invariant to test data volume.
    utils/data/perturb_data_dir_volume.sh \
	data/tamsa/${train_set}_sp_hires
...
if [ $stage -le 4 ]; then
    ...
    mkdir -p exp/tamsa/nnet3${nnet3_affix}/diag_ubm
    temp_data_root=exp/tamsa/nnet3${nnet3_affix}/diag_ubm

    num_utts_total=$(wc -l <data/tamsa/${train_set}_sp_hires/utt2spk)
	...
	--subsample 2 \
	${temp_data_root}/${train_set}_sp_hires_subset \
	exp/tamsa/nnet3${nnet3_affix}/pca_transform
	...
	512 \
	exp/tamsa/nnet3${nnet3_affix}/pca_transform \
	exp/tamsa/nnet3${nnet3_affix}/diag_ubm
fi
...
if [ $stage -le 6 ]; then
    ivectordir=exp/tamsa/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
...
if [ $stage -le 7 ]; then
	...
	steps/online/nnet2/extract_ivectors_online.sh \
	    ...
	    exp/tamsa/nnet3${nnet3_affix}/extractor \
	    exp/tamsa/nnet3${nnet3_affix}/ivectors_${data}_hires
    done
fi
