...
for session in ${sessions1}; do
  echo "$0: Correcting session $session."
...
echo "$0: Sox processing for correcting clock drift"
for session in ${sessions1}; do
  echo "$0: Correcting clock drift in session $session."
...
echo "$0: adjust the JSON files"
mkdir -vp ${odir}/transcriptions/eval ${odir}/transcriptions/dev ${odir}/transcriptions/train
...
pushd ${odir}
echo "$0: check MD5 hash value for generated audios"
