...
echo "$0 $@"
if [ $# -le "4" ]; then
  echo "Usage: $0 <g2p-model> <g2p-tmp-dir> <current-lexicon> <output-lexicon> <INPUT_TEXT_FILES>"
  exit 1
fi
...
cat $input_text | \
  local/g2p/count_oovs.pl $lexicon | \
...
phonetisaurus-apply --nbest $var_counts --model $model --thresh 5 --accumulate --word_list $workdir/missing_onlywords.txt > $workdir/missing_onlywords_g2p_${var_counts}.txt 
phonetisaurus-apply --nbest $var_counts --model $model --thresh 5 --accumulate --word_list $workdir/missing.txt > $workdir/missing_g2p_${var_counts}.txt 
echo "Adding new pronunciations to $lexicon in $outlexicon"
