...
list=$1
# Make the speech activity directory
mkdir -p speechactivity

{
  while read src; do
      base=$(basename $src .$input_extension)
      working_dir=speechactivity/$base
      mkdir -p $working_dir
...
EOF
    ...
    local/get_transform_probs_mat.py --priors=$sad_nnet_dir/post_output.vec --sil-scale=$sil_scale > $working_dir/transform_probs.mat
