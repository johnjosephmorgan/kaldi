...
if [ $stage -le 0 ]; then
  for f in dev-1 dev-2 train; do
    echo "$0: Retrieving $f supervision files."
    mkdir -p data/$f
    find $datadir/$f/sad -type f -name "*.tab" | xargs cat > \
      data/$f/annotation.txt
    echo "$0: Writing utt2lang for $f."
...
if [ $stage -le 1 ]; then
  for d in train dev-1 dev-2; do
    echo "$0: Retrieving paths to audio files."
...
if [ $stage -le 4 ]; then
  for f in dev-1 dev-2 train; do
    echo "$0: Extracting MFCC features for $f."
