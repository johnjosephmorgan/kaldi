...
# chain2 recipe for  Tunisian MSA, globalphone tunisian and gale_arabic.
...
egs_dir_list="$dir/tunisian_msa_processed_egs $dir/globalphone_tunisian_processed_egs $dir/gale_arabic_processed_egs"
lang2weight="0.34,0.33,0.33"
lang_list=(tunisian_msa globalphone_tunisian gale_arabic)
num_langs=3
...
if [ $stage -le 23 ]; then
  ...
  for f in devtest test; do
    ...
    local/buckwalter2unicode.py -r -i ${f}_text.txt -o data/tunisian_msa/test_${f}_bw/text
    paste -d " " ${f}_index ${f}_text > data/tunisian_msa/${f}_hires_bw/text
