...
# devtest_anwar_recordings_make_lists.pl - make acoustic model training lists
...
example:
$0 Tunisian_MSA/data/transcripts/devtest/anwar_recordings.tsv anwar libyan
";
}
...
open my $TXT, '+>', $txt or croak "problem with $txt $!";
 ...
 LINE: while ( my $line = <$WAVLIST> ) {
     ...
     my $speaker = $dirs[-1];
     my $rec_id = $speaker . '_' . 'recording' . '_' . $base;
