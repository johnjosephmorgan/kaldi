...
# extracted.
# It also takes an SAD RTTM file containing segments.
...
def get_args():
    parser = argparse.ArgumentParser(
        description="""This script prepares targets for whole recordings for training
            an SAD detector system. It takes as input a data dir
            where it is assumed that MFCC features have already been extracted. 
It also takes an SAD RTTM file containing segments. It uses these segments to 
    ...
    parser.add_argument("reco2num_frames", type=str,
                        ...
                        """)
    parser.add_argument("sad_rttm", type=str,
                        help="Input RTTM file containing speech and silence segments")
...
def run(args):
    ...
    segments = []
    with common_lib.smart_open(args.sad_rttm) as f:
    ...
    for reco_id in reco2num_frames:
        ...
        sad_vec = np.array([other_val,other_val,target_val], dtype=np.float)
        num_targets = [0,0,0]
